...
import { HeroSectionProps, RichTextNode } from '@/types/pages';
...
export const HeroSection = forwardRef<AnimatedRef, HeroSectionProps>(
  ({ tag, title, description }, ref) => {
            ...
            {description && (
              <p className="text-wrap max-w-150 text-body-md text-center tracking-normal font-inter text-neutral-600 leading-[1.3em]">
                {description?.map((node, i) => {
                  if (node.type === 'lineBreak') {
                    return <br className="hidden" key={`${i}-${node.type}`} />;
                  }
                  return (
                    <span
                      key={`${i}-${node.type}`}
